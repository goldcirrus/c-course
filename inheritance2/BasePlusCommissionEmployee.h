...
public:
    ...
    /*rewrite base class's function earnings(),
      derived class can't use Base class's private data member directly,
      must import base class's function BaseClass::earnings() p522 */
    double earnings() const
    {
        return getBaseSalary()+ CommissionEmployee::earnings();
    }

    //rewrite base class's function print()
    std::string print() const
    {
        std::ostringstream output;
        output<<std::fixed<<std::setprecision(2);
        output<<CommissionEmployee::print()          //import base class's function BaseClass::print()
